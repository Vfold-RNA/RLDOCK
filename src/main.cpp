...
int main(int argc, char** argv ) 
{
    ...
    cmpl.Mol2_Info_Get(P.ligand_file);
    ...
    cout<<"Input conformers: "<<cmpl.conformers_num<<endl;
    ...
    if( !pocket_file || !usepose_file)
    {
        ...
        out_pocket_file<<"/// The data of binding site is displayed in the ascending order of the least geometric_score.                                                                                         ///"<<endl;
        out_pocket_file<<"/// For each binding site, the data of each input conformer is displayed in the ascending order of conformer index.                                                     ///"<<endl;
        out_pocket_file<<"/// For each conformer, the data of each anchored atom is displayed in the ascending order of geometric_score(LJ_potential).                                            ///"<<endl;
        out_pocket_file<<"/// Data format of pocket: pocket_index(int)    x(double)       y(double)      z(double)   distance_to_reference_ligand(double)   LJ_potential(double)                  ///"<<endl;
        out_pocket_file<<"/// Data format of input conformer: conformer_index(int)  anchored_atom_index(int)  geometric_score(double) anchored_atom_index(int)   geometric_score(double) ...      ///"<<endl;
        out_pocket_file<<"///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////"<<endl; 
        out_pocket_file<<" rec_num: "<<cmpl.rec_atom_num<<" lig_num: "<<cmpl.lig_atom_num<<" pocket_num: "<<pockets.size()<<" conformer_num: "<<cmpl.conformers_num<<" "<<cost_t<<" s"<<endl;
        ...
        string pocket_xyz_filename = P.output_prefix+"_pocket.xyz";
        out_pocket_file.open(pocket_xyz_filename.c_str());
        out_pocket_file<<pockets.size()<<endl;
        out_pocket_file<<P.output_prefix<<endl;
        for(auto& a:pockets)out_pocket_file<<left<<"H "<<right<<setw(12)<<setprecision(3)<<a.x<<setw(12)<<a.y<<setw(12)<<a.z<<setw(8)<<endl;
        out_pocket_file.close();   
    ...
    else
    {
        ...
        while(getline(sfl_file,sline))
        {
            if(sline.find("<JobName>")!=string::npos)
            {
                ...
                if(num_ligand_atom!=cmpl.lig_atom_num){cout<<"ligand int _SF_low.dat "<<num_ligand_atom<<" != lig.noH "<<cmpl.lig_atom_num<<endl;exit(3);}
            }
            if(sline.find("<site>")!=std::string::npos)
            {
                ...
                getline(sfl_file,sline);
                top_each_site[isite][irank].position.resize(num_ligand_atom);
        ...
        sfl_file.close();
        cout<<"_SF_low.dat finish reading"<<endl;
    }
    ...
    /////clustering/////
    ////////////////////
    vector<POSE> all_pose_standard;;
    for(auto&a:all_pose){if(a.eng<100000000){all_pose_standard.push_back(a);}}
    cout<<"Clustering start"<<endl;
    vector<POSE> cluster;
    for(int irank=0;irank!=all_pose_standard.size();irank++)
    {
        bool accept_flag=true;
        for(int jrank=0;jrank!=cluster.size();jrank++)
        {
            double rr=0;
            for(int ilig=0;ilig!=cmpl.lig_atom_num;ilig++)
            {
                rr += (all_pose_standard[irank].position[ilig].x - cluster[jrank].position[ilig].x)*(all_pose_standard[irank].position[ilig].x - cluster[jrank].position[ilig].x);
                rr += (all_pose_standard[irank].position[ilig].y - cluster[jrank].position[ilig].y)*(all_pose_standard[irank].position[ilig].y - cluster[jrank].position[ilig].y);
                rr += (all_pose_standard[irank].position[ilig].z - cluster[jrank].position[ilig].z)*(all_pose_standard[irank].position[ilig].z - cluster[jrank].position[ilig].z);
            }
            if(rr < (cmpl.lig_atom_num * 4.0)){accept_flag=false;break;}
        }
        if(accept_flag)cluster.push_back(all_pose_standard[irank]);
    }
    string clu_filename = P.output_prefix+"_cluster.mol2";
    cout<<clu_filename<<endl;
    ofstream out_clu_filename(clu_filename.c_str());
    out_clu_filename<<std::fixed;
    out_clu_filename<<"# JobName: "<<P.output_prefix.substr(found+1)<<endl;
    ...
    for(int irank=0;irank!=output_num;irank++)
    {
        ...
        out_clu_filename<<"# Rank: "<<irank+1<<" RMSD: "<<cluster[irank].rmsd<<endl;
        out_clu_filename<<"# Anchored atom "<<cluster[irank].iatom<<" Conformer Index: "<<cluster[irank].iconf<<endl;
        out_clu_filename<<"# Total_Energy: "<<cluster[irank].eng_rerank<<endl;
        out_clu_filename<<"# LJ_potential: "<<cluster[irank].lj<<endl;
        out_clu_filename<<"# Electrostatic_potential: "<<cluster[irank].ele<<endl;
        out_clu_filename<<"# Polarization: "<<cluster[irank].pol<<endl;
        out_clu_filename<<"# Self_energy_of_ligand: "<<cluster[irank].self_lig<<endl;
        out_clu_filename<<"# SASA: "<<cluster[irank].sasa<<endl;
        out_clu_filename<<"# H_bond: "<<cluster[irank].hb<<endl;
        ...
        out_clu_filename<<cmpl.lig_mol2.mol_type<<endl;
        out_clu_filename<<cmpl.lig_mol2.charge_type<<endl;
        out_clu_filename<<" "<<endl;
        out_clu_filename<<" "<<endl;
        out_clu_filename<<"@<TRIPOS>ATOM"<<endl;
        for(int ia=0;ia!=cmpl.lig_mol2.atoms.size();ia++)
        {
            out_clu_filename<<left<<setw(4)<<ia+1<<setw(5)<<cmpl.lig_mol2.atoms[ia].name<<right<<setw(8)<<setprecision(3)<<cluster[irank].position[ia].x
            <<setw(8)<<cluster[irank].position[ia].y<<setw(8)<<cluster[irank].position[ia].z<<setw(7)<<cmpl.lig_mol2.atoms[ia].type
            <<setw(4)<<cmpl.lig_mol2.atoms[ia].resi_index<<setw(6)<<cmpl.lig_mol2.atoms[ia].resi_name<<setw(8)<<cmpl.conformers[0].noH[ia].q<<endl;
        }
        out_clu_filename<<"@<TRIPOS>BOND"<<endl;
